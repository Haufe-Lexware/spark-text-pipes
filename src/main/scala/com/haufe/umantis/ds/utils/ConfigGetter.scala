...
trait ConfigGetter {
  ...
  def getMainConfig(filename: String = "/main_conf.conf"): Config = {
    val mainConfig = getConfig(filename)
    getConfig(mainConfig.getString("main-conf.conf-file"))
  }

  implicit class ConfigHelper(config: Config) {
    def makeConcise: String = {
      config
        .resolve()
        .root()
...
object ConfigGetter extends ConfigGetter
